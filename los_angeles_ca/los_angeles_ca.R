...
# anything starting in CV, BC, LC, PC, SACV, SC, TA, TC, B, BS, EC, NC is a
case inlitigation.
...
# see
for example Navas, Rosa v City - a number of payments which all add up to $2 .5 m(number confirmed here https: //verdictvictory.com/blog/los-angeles-to-pay-out-more-than-8-million-in-wrongful-death-settlements/)
    ...
    # Filter out 7 rows with amount awarded = 0 LAPD_edit < -LAPD_edit % > % filter(amount_awarded != 0)

    nrow(LAPD_edit % > % group_by_all() % > % filter(n() > 1)) # 0 perfect duplicates

    # Duplicates by claim number ? 62 nrow(LAPD_edit % > % group_by(claim_number) % > % filter(n() > 1))

    # Duplicates by claim number and plaintiff name ? 50 nrow(LAPD_edit % > % group_by(claim_number, matter_name) % > % filter(n() > 1))

    # Duplicates by claim number and fiscal year ? 2 nrow(LAPD_edit % > % group_by(claim_number, fiscal_year) % > % filter(n() > 1)) dups < -LAPD_edit % > % group_by(claim_number, fiscal_year) % > % filter(n() > 1)

    # Same
    case /fiscal year, two different closed-dates. Keeping these

    # What 's filtered out? what'
    s left that ambiguous ?
    table(LAPD_edit$summary_allegations)

    # Closed date missing
    for most of data before 2015 summary(LAPD_edit$closed_date) # missing
    for most before 2015 table(LAPD_edit$fiscal_year) # FY2009 / 10 to FY2019 / 20

    # Create calendar year as second half of fiscal year LAPD_edit$calendar_year_temp < -str_split_fixed(LAPD_edit$fiscal_year, "/", 2)[, 2]
    ...
    # time period of calendar year or incident year or filed year
    if closed date missing summary(LAPD_edit$calendar_year) # Min 2010, max 2020

    # Missing ness of variables print(paste("There are", nrow(LAPD_edit % > % filter(is.na(closed_date))), "rows missing closed date")) print(paste("There are", nrow(LAPD_edit % > % filter(is.na(calendar_year))), "rows missing calendar year")) print(paste("There are", nrow(LAPD_edit % > % filter(is.na(amount_awarded))), "rows missing amount awarded")) print(paste("There are", nrow(LAPD_edit % > % filter(amount_awarded == 0)), "rows with amount awarded = 0")) print(paste("There are", nrow(LAPD_edit % > % filter(is.na(docket_number))), "rows missing docket number")) print(paste("There are", nrow(LAPD_edit % > % filter(is.na(claim_number))), "rows missing claim number"))

    # count cases print("Total number of cases") print(nrow(LAPD_edit))

    print("Total amount awarded") sum(LAPD_edit$amount_awarded)

    # Check with WSJ numbers LAPD_edit_2015_2020 < -LAPD_edit % > % filter(fiscal_year % in % c("FY2015/16",
      "FY2016/17",
      "FY2017/18",
      "FY2018/19",
      "FY2019/20"))
